...
{
    CreateWindow(SELECT_FILE_OPTION_CLASS, TEXT("Select File Option - Fourier"), WS_OVERLAPPED | WS_VISIBLE | WS_SYSMENU, 700, 450, 330, 140, windowHandle, NULL, NULL, NULL);
...
{
    // TODO: check if the file is new, if it is pass control to SaveAs instead.
    WriteWaveFile();
}
...
void PaintNewFileOptionsWindow(HWND windowHandle)
{
   // Adding trackbar-textbox-units triple for selecting file length.
    AddTrackbarWithTextbox(windowHandle, &(newFileOptionsHandles->lengthTrackbar), &(newFileOptionsHandles->lengthTextbox),
        10, FILE_MIN_LENGTH, FILE_MAX_LENGTH, NEW_FILE_DEFAULT_LENGTH, LENGTH_TRACKBAR_SENSITIVITY, TEXT(XStringify(NEW_FILE_DEFAULT_LENGTH)), TEXT("seconds"));
    ...
    AddTrackbarWithTextbox(windowHandle, &(newFileOptionsHandles->frequencyTrackbar), &(newFileOptionsHandles->frequencyTextbox),
        55, FILE_MIN_FREQUENCY, FILE_MAX_FREQUENCY, NEW_FILE_DEFAULT_FREQUENCY, FREQUENCY_TRACKBAR_SENSITIVITY, TEXT(XStringify(NEW_FILE_DEFAULT_FREQUENCY)), TEXT("Hz"));
...
void AddTrackbarWithTextbox(HWND windowHandle, HWND* trackbar, HWND* textbox, int yPos, int minValue, int maxValue, int defaultValue, int sensitivity, LPCTSTR defaultValueStr, LPCTSTR units)
{
    ...
    *trackbar = CreateWindow(TRACKBAR_CLASS, NULL, WS_CHILD | WS_VISIBLE | TBS_HORZ | TBS_AUTOTICKS, 10, yPos, 200, 30, windowHandle, 0, NULL, NULL);
    ...
    SendMessage(*trackbar, TBM_SETRANGEMAX, (WPARAM)TRUE, (LPARAM)(maxValue + 1)); // Configuring max range of trackbar.
    SendMessage(*trackbar, TBM_SETPOS, (WPARAM)TRUE, (LPARAM)defaultValue); // Configuring default selection.
    SendMessage(*trackbar, TBM_SETLINESIZE, 0, (LPARAM)sensitivity); // Configuring sensitivity for arrow key inputs.
    SendMessage(*trackbar, TBM_SETPAGESIZE, 0, (LPARAM)sensitivity); // Configuring sensitivity for PGUP/PGDOWN inputs.
...
void PaintSelectFileOptionWindow(HWND windowHandle)
{
    ...
    CreateWindow(STATIC_CLASS, TEXT("Create a new file or open an existing one?"), WS_VISIBLE | WS_CHILD | SS_CENTER, 0, 30, 330, 30, windowHandle, NULL, NULL, NULL);

    // Adding buttons for new file and open file.
    CreateWindow(BUTTON_CLASS, TEXT("New file"), WS_VISIBLE | WS_CHILD | BS_CENTER | BS_VCENTER, 85, 70, 70, 35, windowHandle, (HMENU)FILE_MENU_NEW, NULL, NULL);
    CreateWindow(BUTTON_CLASS, TEXT("Open file"), WS_VISIBLE | WS_CHILD | BS_CENTER | BS_VCENTER, 175, 70, 70, 35, windowHandle, (HMENU)FILE_MENU_OPEN, NULL, NULL);
}
