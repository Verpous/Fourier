		...
		switch (chunk.id)
		{
			case FOURCC_FORMAT:
				*formatChunk = chunkPos;
				formatChunks++;
				break;
			case FOURCC_LIST:; // When the chunk is a list, we want to check if it is a list of wave data. If it is, this is our wave data chunk. Otherwise we ignore it.
				FOURCC listType;
...
unsigned short GetChannelNames(FileInfo* fileInfo, TCHAR buffer[][CHANNEL_NAME_BUFFER_LEN])
{
	...
	{
		channelMask = relevantChannels == 1 ? SPEAKER_FRONT_CENTER : relevantChannels == 2 ? SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT : 0;
	}
